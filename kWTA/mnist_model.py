...
class SparseMNIST_CNN(nn.Module):
    def __init__(self, sp1, sp2, func, num_channels, hidden_size=20000, bias=True):
        ...
        self.cnn = nn.Sequential(nn.Conv2d(1, num_channels[0], 3, padding=1, bias=bias),
                                 models.sparse_func_dict[func](sp1),
                                 nn.Conv2d(num_channels[0], num_channels[1], 3, padding=1, stride=2, bias=bias),
                                 models.sparse_func_dict[func](sp1),
                                 nn.Conv2d(num_channels[1], num_channels[2], 3, padding=1, bias=bias),
                                 models.sparse_func_dict[func](sp1),
                                 nn.Conv2d(num_channels[2], num_channels[3], 3, padding=1, stride=2, bias=bias),
                                 models.sparse_func_dict[func](sp1),
                                 Flatten(),
...
class SparseMNIST_CNN_BN(nn.Module):
    def __init__(self, sp1, sp2, func, num_channels, hidden_size=20000, bias=True):
        ...
        self.cnn = nn.Sequential(nn.Conv2d(1, num_channels[0], 3, padding=1, bias=bias), nn.BatchNorm2D(num_channels[0]),
                                 models.sparse_func_dict[func](sp1),
                                 nn.Conv2d(num_channels[0], num_channels[1], 3, padding=1, stride=2, bias=bias),
                                 nn.BatchNorm2D(channels[1]), models.sparse_func_dict[func](sp1),
                                 nn.Conv2d(num_channels[1], num_channels[2], 3, padding=1, bias=bias),
                                 nn.BatchNorm2D(channels[2]), models.sparse_func_dict[func](sp1),
                                 nn.Conv2d(num_channels[2], num_channels[3], 3, padding=1, stride=2, bias=bias),
                                 nn.BatchNorm2D(channels[3]), models.sparse_func_dict[func](sp1),
                                 Flatten(),
                                 nn.Linear(7 * 7 * num_channels[3], hidden_size), models.Sparsify1D(sp2),
...
class SparseDNN(nn.Module):
    def __init__(self, hidden_size=20000, sp=0.5, bias=True, norm_factor=1, kact=False):
        ...
        if kact:
            self.nn = nn.Sequential(Flatten(), nn.Linear(28 * 28, hidden_size, bias=bias),
                                    models.Sparsify1D_kactive(sp),
